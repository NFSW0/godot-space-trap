## 界面管理器
## 主控层(游戏菜单、全屏幕独占-阻止输入传递到后续UI-同一时间只能有一个主控UI)>输入层(聊天栏、同一时间只能有一个UI获得焦点)>显示层(数据显示、不可交互)
...
## 第三层：占据部分屏幕，不会阻止输入到后续层(也就是游戏世界)，注册在第三层的UI可以同时存在，无需 处理覆盖关系，通常用于显示信息。使用案例：血条显示、名称显示……
extends CanvasLayer
